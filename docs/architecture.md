...
- 首期仅通过 SMTP 发送邀请和密码重置邮件；不实现风险预警邮件、企业微信、SSO 或合同履约提醒。
...
| 模型 | 默认 DeepSeek `deepseek-v4-flash`，经 `ModelGateway` 适配层调用；Qwen 可选 | 隔离供应商协议、重试和结构化输出校验 |
...
| `integrations` | 模型 Provider、OCR、文件存储、病毒扫描、通知适配 | 领域状态决策 |
...
sequenceDiagram
    ...
    participant M as 默认模型 Provider
...
| `reports` | `organization_id`, `review_task_id`, `display_no`, `format`, `status`, `snapshot_json`, `template_version`, `file_object_id`, `created_at`, `updated_at`, `started_at`, `finished_at`, `generated_at`, `expires_at`, `error_code`, `error_message` | `status` 为 `generating|ready|failed|expired`；`snapshot_json`、任务/格式/模板字段不可变；ready 到 `now >= expires_at` 转 expired；重新生成创建新记录，不覆盖旧文件；同任务/格式仅允许一个 generating |
...
| 类别 | 示例 | 策略 |
| --- | --- | --- |
| 可重试瞬时错误 | 模型 Provider 429/5xx、连接超时、临时存储错误 | 指数退避加抖动，遵守 `Retry-After`，达到配置上限后失败 |
...
### 10.2 模型 Provider 适配层

`ModelGateway` 暴露四个明确方法：合同分类、结构化要素抽取、风险语义分析、条款比对/建议生成。业务服务传入版本化请求对象，不直接引用任何供应商 SDK 类型。
...
- OCR 失败页与低置信度页；
- 模型 Provider 请求量、错误、token、延迟和组织预算消耗；
- 预警新增、未处理时长、关闭率和误报率；
...
| 模型契约测试 | Provider 适配器 Fake + 固定响应样本 | JSON Schema、非法 JSON、超时、429、证据缺失和版本记录；普通 CI 不调用付费 API |
...
- Provider 超时、限流、5xx、非法 JSON、Schema 不符和返回重复风险的固定响应。
...
每次合并依次执行格式/静态检查、后端单元与集成测试、前端测试、数据库迁移升级测试、镜像构建和关键 E2E。主分支定期执行完整文档金样和离线评测；付费模型冒烟测试只在受保护环境手工触发，设置调用预算。
...
前端构建产物由反向代理提供，浏览器只访问一个域名。API 健康检查拆为：`/health/live` 只检查进程，`/health/ready` 检查数据库和关键配置；模型 Provider 短暂不可用不应让 API 进程退出就绪，而应反映为依赖状态和任务错误。
...
至少区分 `local/test/staging/production`。非敏感配置使用环境变量和组织配置表；模型 Provider 密钥、会话密钥、数据库密码和对象存储凭据使用部署平台 Secret 注入。启动时校验必需配置。Worker、Scheduler、Migration 和其他非 HTTP 进程遇到错误时立即退出，并只给出不含秘密的配置名；API 进程可以启动仅挂载健康检查的降级实例，使 `live` 继续反映进程存活、Internal `ready` 安全返回 `503 SERVICE_NOT_READY`，不得在配置无效时挂载业务路由。
...
- 默认模型分类、五项必备字段、10 类以上风险、基础条款比对和证据定位。
...
| 项目 | 建议默认值 | 确认方 |
| --- | --- | --- |
| 模型 Provider 与套餐 | 默认使用 DeepSeek `deepseek-v4-flash`，名称由环境变量注入；Qwen 可选 | 产品/部署方 |
