...
`storage_status`：`quarantine | stored | deleting | deleted | failed`。`deleting` 和 `deleted` 仅表示 FileStore 内容清理状态，数据库 `FileObject` 元数据保留；非 `stored` 状态不可下载。
...
业务预警首期只实现站内通知；通知投递失败记录在通知资源中，不回滚预警。站内通知的 `delivered` 只表示通知事实已成功持久化，自动补偿只处理已有 `failed` 投递事实，不创建重复通知；补偿最多 3 次（包含首次投递），第 2 次在首次失败后 1 分钟、第 3 次在第二次失败后 5 分钟执行；达到上限后保持 `failed`、清空 `next_attempt_at` 并保留最终失败事实，不提供手工重试 API。SMTP 仅用于身份邀请和密码重置，不属于本模块；风险预警邮件和企业微信仅保留适配边界。
...
合同使用 archive/restore，不提供物理 `DELETE /contracts/{id}`。规则/模板发布版本、审核结果、报告快照、人工修订、预警事件、反馈和审计日志不提供删除接口。文件内容进入软删除和保留期清理流程；被历史审核、报告或审计引用的数据库历史事实和 `FileObject` 元数据不得物理删除，只有通过引用/活动任务检查的 FileStore 内容可以清理。

组织/报告默认内容保留期为 180 天，审计日志保留期为至少 365 天；365 天不覆盖审计不可变语义，Phase 14B 不删除审计事实。生产应用日志保留 30 天，本地开发日志保留 7 天。清理任务必须先检查历史引用、活动租约和补偿状态，并为清理、跳过、重试、恢复和最终失败写安全审计。
